    ...
    BEGIN
        ...
        ELSIF RISING_EDGE(CLK) THEN
            ...
            CASE STATE IS
                ...
                WHEN WAIT_PIN =>
                    -- Mantener START_CONV='1' hasta que ADS1115 acepte (BUSY='1')
                    -- Esto evita la race condition al startup donde el I2C driver
                    -- aun tiene BUSY='1' por ~125 ciclos despues del reset.
                    IF BUSY = '1' THEN
                        START_CONV <= '0';
                    END IF;
                    IF CONV_READY = '1' THEN
                        START_CONV <= '0';
                        DATA_VALID <= '1';  -- Pulso de dato valido
                        STATE <= IDLE;
                    END IF;
            END CASE;
        END IF;
